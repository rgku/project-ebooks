...
def test_fallback_on_flux_error(tmp_path):
    orig = os.getcwd()
    os.chdir(tmp_path)
    try:
        with patch("cover_ai.config.get_api_key", return_value="sk-test"):
            with patch("cover_ai.requests.post", side_effect=Exception("API down")):
                import cover_ai
                out = str(tmp_path / "cover.png")
                cover_ai.generate_cover(_niche(), out)
                assert os.path.exists(out)
